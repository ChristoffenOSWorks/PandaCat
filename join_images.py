...
def joinImages1():
	...
	while (pointer_dec < number_of_times):
		...
		imgNames = raw_input(spaces.space3 + bell)

		name = imgNames
	
		with open(filename, 'a') as f:
			print >> f, spaces.space1 + "cairo_surface_t *" + str(name) + ";\n"
		f.close()
		pointer_dec += 1
	...
	number_of_times2 = int(raw_input(spaces.space3 + bell))
	pointer_dec2 = 0
	...
	while (pointer_dec2 < number_of_times2):
		...
		print "Enter image nickname"
		print spaces.space3 + subHeading * length12
		imgNames2 = raw_input(spaces.space3 + bell)

		print new + spaces.space3 + subHeading * length12
		print "Enter name of image png file"
		print spaces.space3 + subHeading * length12
		pngTitle = raw_input(spaces.space3 + bell)

		name2 = imgNames2
		title = pngTitle
		with open(filename, 'a') as f:
			print >> f, spaces.space1 + "\ncairo_push_group (cr);"
			print >> f, spaces.space1 + str(name2) + " " + heading + " " + "cairo_image_surface_create_from_png (" + quote + str(title) + quote + ");"
			print >> f, spaces.space1 + "cairo_pop_group_to_source (cr);"
		f.close()
		pointer_dec2 += 1

	print "More nicknames, same order"
	number_of_times3 = int(raw_input(spaces.space3 + bell))
	pointer_dec3 = 0

	while (pointer_dec3 < number_of_times3):
		pngTitle3 = raw_input(spaces.space3 + bell);
		title3 = pngTitle3
		with open(filename, 'a') as f:
			print >> f, "cairo_set_source_surface (cr, " + str(title3) + ", 0, 0);"
			print >> f, "cairo_paint (cr); "
		f.close()
		pointer_dec3 += 1

	print "More nicknames, same order"
	number_of_times4 = int(raw_input(spaces.space3 + bell))
	pointer_dec4 = 0

	while (pointer_dec4 < number_of_times4):
		pngTitle4 = raw_input(spaces.space3 + bell);
		title4 = pngTitle4
		with open(filename, 'a') as f:
			print >> f, "cairo_surface_destroy (" + str(title4) + );"
		f.close()
		pointer_dec3 += 1
